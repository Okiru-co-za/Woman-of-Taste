...
import { Calendar, MapPin, ArrowLeft, Users, CheckCircle, XCircle, Phone, Mail, MessageCircle, Landmark } from "lucide-react";
...
interface Booking {
  ...
  eventId: string; eventTitle: string; eventDate: string; eventLocation: string;
...
export default function EventDetail() {
  ...
  const [tab, setTab] = useState<"guests" | "attendance" | "finance" | "payment">("guests");
  ...
  const eventId: string | null = bookings[0]?.eventId ?? null;

  useEffect(() => {
    if (!eventId) { setBankLoading(false); return; }
    setBankLoading(true);
      ...
      <div style={{ maxWidth: 1000 }}>
        ...
        <div style={{ background: "white", borderRadius: 12, border: "1px solid #eee", overflow: "hidden" }}>
          <div style={{ display: "flex", borderBottom: "1px solid #eee", padding: "0 0.5rem" }}>
            ...
            <button style={TAB_STYLE(tab === "payment")} onClick={() => setTab("payment")}>Payment Details</button>
          </div>
          ...
          <div style={{ padding: "1rem 1.25rem" }}>
            ...
            {!loading && tab === "payment" && (
              <div>
                {!eventId && (
                  <p style={{ fontFamily: "Raleway, sans-serif", color: "#aaa", fontSize: "0.85rem" }}>
                    No bookings yet for this event, so there's no event ID to attach bank details to.
                  </p>
                )}
                {eventId && bankLoading && (
                  <div style={{ fontFamily: "Raleway, sans-serif", color: "#aaa", fontSize: "0.85rem" }}>Loading…</div>
                )}
                {eventId && !bankLoading && (
                  <div style={{ maxWidth: 520 }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: "1.25rem" }}>
                      ...
                      {bankSaved && <span style={{ fontFamily: "Raleway, sans-serif", fontSize: "0.8rem", color: "#16a34a" }}>Saved</span>}
                    </div>
                  </div>
                )}
              </div>
